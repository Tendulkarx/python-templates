...
def gear4(session, headers):
    ...
    data = {"store": "gear4music", "price": float(soup.select_one("span.info-row-price span.c-val").text)}
    return data


def amazon(session, headers):
    url = "https://www.amazon.co.uk/dp/B09738CV2G"
    resp = session.get(url, headers=headers)
    soup = BeautifulSoup(resp.text, "html.parser")
    data = {"store": "amazon", "price": float(soup.select_one("div.celwidget span.a-offscreen").text.replace("£", ""))}
    return data
...
def thomann(session, headers):
    ...
    resp = session.get(url, headers=headers)
    soup = BeautifulSoup(resp.text, "html.parser")
    data = {"store": "thomann", "price": float(soup.select_one("div.price-wrapper div.price").text.replace("£", "").strip())}
    return data
